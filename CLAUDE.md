...
`07_paper/0_result_summaries.md` 是**最终论文的唯一数据源**，必须遵守：

| 规则 | 说明 |
|------|------|
| **只保留最终方案** | 弃用的方法必须删除，不要累积 |
| **结果变更时同步更新** | 任何分析结果变化后立即更新此文件 |
| **格式规范** | 按 样本→主分析→敏感性 的结构组织 |

> ⚠️ **结果变了 → 0_result_summaries.md 必须同步更新!**
