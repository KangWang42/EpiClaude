...
- PPT、论文、标书、报告和网页的非统计视觉资产必须先触发 `research-visuals` → `imagegen`，先按载体、读者、信息功能、证据属性和实际显示尺寸选择视觉语言，再生成整张图。流程、框架、机制、路线和图形摘要由 AI 直接生成包含完整文字、数字和箭头的整图；文字密集、图形看似适合 SVG、当前可见技能清单未列出 `research-visuals`，均不是提前跳过 imagegen 的理由。全新生成和重生成不得携带参考图，不设置 `referenced_image_paths` 或 `num_last_images_to_include`；参考图只查看、解构并转写进完整提示词。仅编辑一张既有目标图片时可传入该目标图，不附加其它风格参考图。不得用 Python、PPT/Word 文本框或 SVG 覆盖层给 AI 图补字。真实统计图走 `publication-figures`，科研原始图像不得生成式重绘；只有明确要求矢量、实际工具不可用或整图经 imagegen 修正后仍不准确时才回退 `svg-diagrams`。
