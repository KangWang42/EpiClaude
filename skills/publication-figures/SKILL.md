...
3. **标签领域化**：轴/图例/注释/分面标题**只用领域可读名**，**NEVER** 暴露内部变量名、列前缀、单位后缀（`traj_`、`seq_`、`_ms2`、`_bpm`、`subject_id`）；维护"列名→中文标签"映射，新变量必补条目（呼应 CLAUDE §2 禁暴露内部名）。
...
3. **表达正确**：分类映射无 NA、图例不重复不缺、标签无内部变量名、坐标/标签/标题无裁切、图型匹配数据（AUC 用点+CI 而非截断柱）、多结局图含全部结局、与同篇其它图 theme/配色/布局一致。
