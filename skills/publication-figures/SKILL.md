...
- [ ] **同一论文各图风格体系一致**（字体、配色角色、线宽、图例、背景/底纹逻辑和信息密度一致，不同图型允许不同布局）；**多结局分析每张图含全部结局**，不漏
...
4. **全文风格体系一致 + 不漏结局**：同一论文所有图统一字体、配色角色、线宽、图例、背景或底纹逻辑与信息密度；不同图型允许采用不同布局，不得为形式一致牺牲表达清晰度。**多结局/多终点分析里每张按结局的图必须含全部结局**，不可只画其中几个；**亚组森林必须按亚组类别分区呈现**（`facet_grid(类别~)` 或分区标题），不可平铺成无结构列表。
