...
【验证】
  ...
  - 主效应值在 [合理范围]
  - PH 假设 / 线性假设 / 收敛性如何判定
  - 若存在计算密集的独立重复：记录自动检测的可用线程、worker、内层线程和串并行等价性检查
```
...
- **计算资源遵 [../biostat-principles/references/compute-resources.md](../biostat-principles/references/compute-resources.md)**：bootstrap、置换、模拟、重采样、交叉验证和独立模型批次先判断是否值得并行；运行时检测可用逻辑线程并保留系统余量，外层并行时把每个 worker 的模型 / BLAS 内层线程限制为 1，随机流必须可复现。
...
- [ ] 计算密集的独立重复已使用自适应 worker，或已记录保持串行的理由；并行结果与固定小样本串行基线一致，且没有重复或缺失任务
