...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by Reviewer to validate
        try:
            ...
            # Check for w:del or w:ins tags authored by Reviewer
            del_elements = root.findall(".//w:del", self.namespaces)
            ins_elements = root.findall(".//w:ins", self.namespaces)

            # Filter to only include changes by Reviewer
            reviewer_del_elements = [
                elem
                for elem in del_elements
                if elem.get(f"{{{self.namespaces['w']}}}author") == "Reviewer"
            ]
            reviewer_ins_elements = [
                elem
                for elem in ins_elements
                if elem.get(f"{{{self.namespaces['w']}}}author") == "Reviewer"
            ]

            # Redlining validation is only needed if tracked changes by Reviewer have been used.
            if not reviewer_del_elements and not reviewer_ins_elements:
                if self.verbose:
                    print("PASSED - No tracked changes by Reviewer found.")
                return True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Remove Reviewer's tracked changes from both documents
            self._remove_reviewer_tracked_changes(original_root)
            self._remove_reviewer_tracked_changes(modified_root)
            ...
            if self.verbose:
                print("PASSED - All changes by Reviewer are properly tracked")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """Generate detailed word-level differences using git word diff."""
        error_parts = [
            "FAILED - Document text doesn't match after removing Reviewer's tracked changes",
            "",
            "Likely causes:",
    ...
    def _remove_reviewer_tracked_changes(self, root):
        """Remove tracked changes authored by Reviewer from the XML root."""
        ...
        for parent in root.iter():
            to_remove = []
            for child in parent:
                if child.tag == ins_tag and child.get(author_attr) == "Reviewer":
                    to_remove.append(child)
            for elem in to_remove:
                parent.remove(elem)

        # Unwrap content in w:del elements where author is "Reviewer"
        ...
        for parent in root.iter():
            to_process = []
            for child in parent:
                if child.tag == del_tag and child.get(author_attr) == "Reviewer":
