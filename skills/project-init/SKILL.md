...
name: project-init
description: |
  标准化卫生统计研究项目初始化 skill。一键创建七层目录结构、模板文件与可选 Git 配置；支持"研究模式"和"咨询模式"双预设。Git 不可用时跳过版本管理，不安装 Git，也不影响项目骨架创建。
...
4. 存放根路径（默认当前工作目录）：
5. 是否启用 Git（仅在 Git 已可用时询问，默认 yes；不可用时自动跳过且不安装）：
```
...
`git = TRUE` 只表示“可用时启用”。脚本先用 `Sys.which("git")` 检测；未找到 Git 时继续创建项目并明确报告跳过，不执行安装。
