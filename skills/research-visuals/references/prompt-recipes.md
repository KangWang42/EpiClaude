...
每轮只锁定一个主要问题，但必须用包含全部结构和不变量的纯文本提示词从零生成整图，不附带上一版或任何参考图：

```text
Generate a new complete image from text only. Do not use or condition on any reference image.
Primary issue to fix: <单一问题>.
Reproduce exactly: <载体比例、完整区域、全部节点与边、逐字标签、三条路径、结果栏等不变量>.
Keep unchanged: composition, subject identity, object count, reading order, palette, lighting, materials, safe zones, and all exact labels and relationships.
Do not add, delete, move, restyle, or rewrite unrelated elements. Render the entire image, not a patch.
```

修正后重新检查整张原图，而不是只看问题区域。最多连续两次纯文本整图重生成。参考图条件生成发生超时或失败时，不重复该路径，也不把它误判为 imagegen 整体不可用；直接把全部内容合同重写进纯文本提示词。精确结构图仍不准确时整图回退 `svg-diagrams`，不得用 Python、PPT/Word 文本框或 SVG 覆盖层补字。
